...
def _profile_entry_from_metadata(
    profile_name: str, metadata: Dict[str, Any], source_label: str
) -> Dict[str, Any]:
    """Build a list-entry dict from parsed frontmatter metadata."""
    return {
        "name": profile_name,
...
def _scan_directory(directory: Path, source_label: str, profiles: Dict[str, Dict]) -> None:
    ...
    for item in directory.iterdir():
        if item.is_dir():
            ...
            if agent_md.exists():
                try:
                    metadata = dict(frontmatter.loads(agent_md.read_text()).metadata)
                except Exception:
                    pass
            if profile_name not in profiles:
                profiles[profile_name] = _profile_entry_from_metadata(
                    profile_name, metadata, source_label
                )
        ...
        for item in agent_store.iterdir():
            ...
            if name.endswith(".md"):
                ...
                try:
                    ...
                    pass
                profiles[profile_name] = _profile_entry_from_metadata(
                    profile_name, metadata, "built-in"
                )
