...
def _message_from_session_comment(
    agent_session: Mapping[str, Any],
    *,
    metadata: Optional[Mapping[str, Any]] = None,
    refs: ProviderRefFactory = LINEAR_REFS,
    ...
    if not body:
        return None
    comment_id = _string_value(comment.get("id")) or f"agent-session:{session_id}:comment"
    return ConversationMessage(
        kind="prompt",
        body=body,
        ref=refs.ref(comment_id),
        direction="inbound",
        state="received",
        metadata=dict(metadata) if metadata is not None else None,
    )
...
def _message_body_from_session_comment(comment: Mapping[str, Any]) -> Optional[str]:
