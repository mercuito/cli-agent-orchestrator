...
The handoff's slice reference must point at a real `feature-tasks.md` entry whose
...
**Bad — broken reference.** Handoff says "see `../feature-tasks.md#t-7`" but
`feature-tasks.md` has no `t-7` entry (only `t-1` through `t-6`).
...
**Bad — references an incomplete entry.** `feature-tasks.md#t-3` has empty Test
