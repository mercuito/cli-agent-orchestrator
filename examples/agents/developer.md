...
1. **Handoff (blocking)**: The message starts with `[CAO Handoff]` and includes the supervisor's agent ID. The orchestrator automatically captures your output when you finish. Just complete the task, present your deliverables, and stop. Do NOT call `send_message` — the orchestrator handles the return.
2. **Assign (non-blocking)**: The message includes a callback agent ID (e.g., "send results back to agent abc123"). When done, use the `send_message` MCP tool to send your results to that agent ID.

Your own agent ID is available in the `CAO_AGENT_ID` environment variable.
